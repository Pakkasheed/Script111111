...
AutoFarm = false--ถ้าเยอกว่านั้นใช้ ใส่ , นี้ด้วย เช่น
--[[
DzXSave = false,
DzXSave2 = false
]]
       ...
       local filename = "Piwwy Save.txt" --File Name ชื่อ ไฟล์ ที่จะตั้งเช่น DzX Save.txt
       ...
       loadsetting() -- คือใช้มันโหลด คอนฟิค
...
--------------------------------------------
local repo = 'https://raw.githubusercontent.com/Pakkasheed/UILinoriaPak/main/'

local Library = loadstring(game:HttpGet(repo .. 'UIPakHub.lua'))()
local Window = Library:CreateWindow({
    Title = 'Piwwy lua Test Hub',
    Center = true, 
    AutoShow = true,
})
local Tabs = {
...
local AutoFarmBoss = GeneralBox2:AddTab('Bosses')
local SettingGeneral = SettingBox:AddTab('Setting')
local SettingGeneral2 = SettingBox2:AddTab('Player Misc')
...
SettingGeneral2:AddLabel('  --------- Auto Stats ---------')
SettingGeneral2:AddToggle('MeleeStats', {
    Text = 'Auto Melee',
    Default = true,
})
Toggles.MeleeStats:OnChanged(function(t)
   Melee = t
   
end)


SettingGeneral2:AddToggle('DefenseStats', {
    Text = 'Auto Defense',
    Default = false,
})
Toggles.DefenseStats:OnChanged(function(t)
   Defense = t
end)

SettingGeneral2:AddToggle('SwordStats', {
    Text = 'Auto Sword',
    Default = false,
})
Toggles.SwordStats:OnChanged(function(t)
   Sword = t
end)

SettingGeneral2:AddToggle('GunStats', {
    Text = 'Auto Gun',
    Default = false,
})
Toggles.MeleeStats:OnChanged(function(t)
...
SettingGeneral2:AddToggle('FruitStats', {
    Text = 'Auto Blox Fruit',
    Default = false,
})
Toggles.FruitStats:OnChanged(function(t)
   Fruit = t
end)
SettingGeneral2:AddSlider('Point', {
    Text = 'Amont Point',
...
local Number = Options.Point.Value
Options.Point:OnChanged(function(t)
    Point = t
end)
...
AutoFarmBoss:AddDropdown('SelectBoss', {
    Values = BossList,
    Default = nil,
    Multi = false,
    Text = 'Select Bosses',
			...
			if Bringmob then
				if AutoFarm then
					if game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == true then
				                        	...
				                        	click()
						for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
							if v.Name == Name and (v.HumanoidRootPart.Position-_G.PosMon.Position).Magnitude <= 350 then
